...
calc_acc(actual = mytr.test$case, predicted = predict(elasticNet, newdata = mytr.test))
...
ncol(myrftr.train)
observations = as.matrix(myrftr.train[,1:109])
forestModel = randomForest(as.matrix(myrftr.train[,1:109]), as.factor(myrftr.train$case), ntree = 250)
forestModel.pred = predict(forestModel, newdata = myrftr.test[,1:109])
forestModel.pred
